...
# Load the dataset
file_path = 'Fall_Trackman_Master.csv'  # Replace with the correct path in your Streamlit setu
@st.cache_data
...
def calculate_hitter_stats(df):
    ...
    df['PA'] = df.groupby('Hitter')['PitchOfPA'].transform(lambda x: (x == 1).sum())

    # At Bats (AB) - Exclude BB, HBP, Sacrifice Fly/Bunt
    ab_df = df[~df['PitchCall'].isin(['BallCalled', 'HitByPitch', 'Sacrifice', 'SacrificeFly'])]
    df['AB'] = ab_df.groupby('Hitter')['PitchOfPA'].transform(lambda x: (x == 1).sum())
    ...
    # Total Hits
    df['Hits'] = hits_df.groupby('Hitter')['PlayResult'].transform('count')

    # Home Runs (HR)
    df['HR'] = df[df['PlayResult'] == 'HomeRun'].groupby('Hitter')['PlayResult'].transform('count')

    # Strikeouts (SO)
    df['SO'] = df[df['KorBB'] == 'Strikeout'].groupby('Hitter')['KorBB'].transform('count')

    # Walks (BB)
    df['BB'] = walks_df.groupby('Hitter')['KorBB'].transform('count')
    ...
    df['1B'] = hits_df[hits_df['PlayResult'] == 'Single'].groupby('Hitter')['PlayResult'].transform('count')
    df['2B'] = hits_df[hits_df['PlayResult'] == 'Double'].groupby('Hitter')['PlayResult'].transform('count')
    df['3B'] = hits_df[hits_df['PlayResult'] == 'Triple'].groupby('Hitter')['PlayResult'].transform('count')
...
columns_to_display = ['Hitter', 'PA', 'AB', 'AVG', 'OBP', 'SLG', 'OPS', 'SO', 'BB', 'HR']
