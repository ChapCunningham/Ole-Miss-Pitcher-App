...
def plot_exit_velocity_heatmap(pitcher_name, batter_side, strikes, balls, date_filter_option, selected_date, start_date, end_date):
    try:
        ...
        # Remove rows where PlateLocSide, PlateLocHeight, or ExitSpeed is NaN
        plot_data = pitcher_data.dropna(subset=['PlateLocSide', 'PlateLocHeight', 'ExitSpeed'])
        ...
        for i, (ax, pitch_type) in enumerate(zip(axes, unique_pitch_types)):
            ...
            # Ensure enough data points exist for a heatmap
            if len(pitch_type_data) >= 5:
                sns.kdeplot(
                    x=pitch_type_data['PlateLocSide'], 
                    y=pitch_type_data['PlateLocHeight'], 
                    weights=pitch_type_data['ExitSpeed'],  # Weight by ExitSpeed
                    fill=True, 
                    cmap='coolwarm', 
                    levels=6, 
                    ax=ax,
            ...
            # Plot scatter points with conditional coloring based on ExitSpeed
            ...
            ax.set_title(f"{pitch_type} ({pitcher_name})", fontsize=18)  # Font size adjusted
        ...
        plt.suptitle(f"{pitcher_name} Exit Velocity Heatmap ({season} Season)", 
        ...
        st.write(f"Error generating Exit Velocity heatmaps: {e}")
